...
<c:set target="${breadCrumbsParams}" property="${idCat}"
	value="${idCategory}" />
	...
	<div id="container_der">
		...
		<div id="migas">
			...
			<ol class="breadcrumb">
				...
				<li><html:link forward="editSeedCategory"
						name="breadCrumbsParams">
						<bean:message key="migas.modificar.categoria" />
					</html:link></li>
				<li class="active"><logic:equal name="accion"
						value="<%= actionNewCS %>">
						<bean:message key="migas.nueva.semilla.observatorio" />
					</logic:equal> <logic:notEqual name="accion" value="<%= actionNewCS %>">
		...
		<div id="cajaformularios">
			...
			<html:form styleClass="formulario form-horizontal" method="post"
				action="/secure/CategorySeedAction"
				onsubmit="return validateSemillaForm(this)">
				...
				<input type="hidden" name="<%=Constants.ID_CATEGORIA %>"
					value="<%=idCategory %>" />
				<fieldset>
					...
					<div class="formItem">
						<label for="nombre" class="control-label"><strong
							...
							<bean:message key="editar.semilla.observatorio.nombre" /></strong></label>
					...
					<div class="formItem">
						<label for="activa" class="control-label"><strong
							class="labelVisu"><acronym
								title="<bean:message key="campo.obligatorio" />"> * </acronym>
							<bean:message key="editar.semilla.observatorio.activa" /></strong></label>
					...
					<logic:notEmpty name="<%=Constants.SEED_AMBITS %>">
						<div class="formItem">
							...
							<html:select property="ambito.id"
								...
								<logic:iterate name="<%=Constants.SEED_AMBITS %>" id="ambit">
									...
									<html:option value="<%=idAmbit %>">
										<bean:write name="ambit" property="name" />
									</html:option>
								</logic:iterate>
							</html:select>
						</div>
					...
					<logic:notEmpty name="<%=Constants.SEED_COMPLEXITIES %>">
						<div class="formItem">
							<label for="complejidad" class="control-label"><strong
								class="labelVisu"><bean:message
										key="nueva.semilla.webs.complejidad" /></strong></label>
							<html:select property="complejidad.id"
								styleClass="textoSelect form-control" styleId="complejidad">
								<html:option value=""> - <bean:message
										key="select.one.femenine" /> - </html:option>
								<logic:iterate name="<%=Constants.SEED_COMPLEXITIES %>" id="complexity">
									<bean:define id="idComplexity">
										<bean:write name="complexity" property="id" />
									</bean:define>
									<html:option value="<%=idComplexity %>">
										<bean:write name="complexity" property="name" />
									</html:option>
								</logic:iterate>
							</html:select>
						</div>
					...
					<div class="formItem">
						...
						<label for="listaUrlsString" class="control-label"><strong
							class="labelVisu"><acronym
								title="<bean:message key="campo.obligatorio" />"> * </acronym>
							<bean:message key="editar.semilla.observatorio.url" /></strong></label>
